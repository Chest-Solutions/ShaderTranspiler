...
// --- [GLOBAL VARIABLES AND HELPER STRUCTS FIRST] ---

static std::atomic<bool> glslAngInitialized = false;
static std::atomic<bool> tintInit = false;
static std::mutex tintInitMtx;

// The struct that our helper functions will return.
// By defining this first, the compiler knows what it is.
...
// These are all the functions that do the actual work. They are now at the top
// of the file so that the main API functions can call them.
...
// --- YOUR UPGRADED CompileGLSLFromFile FUNCTION ---
...
// ... (Rest of the original helper functions: SPIRVToOpenGL, SPIRVToHLSL, etc.)
...
IMResult SPIRVToDXIL(const spirvbytes& bin, const Options& opt, spv::ExecutionModel model){
	...
	auto compileWithNewDxc = [&] {
		...
		switch (model) {
		case decltype(model)::ExecutionModelVertex: profile = L"vs_6_1"; break;
		case decltype(model)::ExecutionModelFragment: profile = L"ps_6_1"; break;
		case decltype(model)::ExecutionModelGLCompute: profile = L"cs_6_1"; break;
		default: throw runtime_error("Invalid shader model");
		}
	...
	auto compileWithOldDxc = [&]() {
		...
		switch (model) {
		case decltype(model)::ExecutionModelVertex: profile = "vs_5_0"; break;
		case decltype(model)::ExecutionModelFragment: profile = "ps_5_0"; break;
		case decltype(model)::ExecutionModelGLCompute: profile = "cs_5_0"; break;
		default: throw runtime_error("Invalid shader model");
		}
		ID3DBlob* code = nullptr;
		ID3DBlob* errormsg = nullptr;
		auto result = D3DCompile( hlsl.sourceData.data(), hlsl.sourceData.size(), "ST_HLSL.hlsl", nullptr, nullptr, "main", profile, D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &code, &errormsg );
		if (result == S_OK) {
...
IMResult SPIRVtoMSL(const spirvbytes& bin, const Options& opt, spv::ExecutionModel model){
    ...
    constexpr auto rgl2sc = [](Options::BindlessSettings::Type type) -> spirv_cross::SPIRType::BaseType{
        switch(type){
            ...
            default: throw std::runtime_error("rgl2sc: invalid type");
        }
    };
    ...
    constexpr auto resourceRenumber = [](auto&& res, auto&& opt, const std::string_view tag, auto&& renumberFn){
        constexpr auto atoi_n = [](std::string_view str) { int ret = 0; for(int i = 0; i < str.size(); ++i){ ret = ret * 10 + (str[i] - '0'); } return ret; };
        auto taglen = tag.size();
        size_t itr = 0;
        while (itr < res.size()){
            itr = res.find(tag.data(), itr, taglen);
            if (itr == std::string::npos){ break; }
            ...
            if (value > 31){ throw std::runtime_error("Adjusted index too large: " + std::to_string(value)); }
    ...
    if (model == spv::ExecutionModel::ExecutionModelVertex && opt.bufferBindingSettings.stageInputSize > 0){
        resourceRenumber(res,opt,"[[buffer(",[&opt](auto value){ return value + opt.bufferBindingSettings.stageInputSize; });
    }
    {
        uint32_t samplerIdx = 0;
        resourceRenumber(res,opt,"[[sampler(",[&opt,&samplerIdx](auto value){ return samplerIdx++; });
    }
...
IMResult SPIRVToWGSL(const spirvbytes& bin, const Options& opt, spv::ExecutionModel model) {
	...
	tintInitMtx.unlock();
	auto tintprogram = tint::reader::spirv::Parse(bin, { .allow_non_uniform_derivatives = true });
...
spirvbytes OptimizeSPIRV(const spirvbytes& bin, const Options &options){
	...
	spvtools::MessageConsumer consumer = [&](spv_message_level_t level, const char* source, const spv_position_t& position, const char* message){
		switch(level){
            ...
            case SPV_MSG_WARNING: case SPV_MSG_INFO: case SPV_MSG_DEBUG: break;
        }
	};
...
static APIConversion ShaderStageToInternal(ShaderStage api) {
	APIConversion cv;
	switch (api) {
	case ShaderStage::Vertex: cv.type = EShLangVertex; cv.model = decltype(cv.model)::ExecutionModelVertex; break;
	case ShaderStage::Fragment: cv.type = EShLangFragment; cv.model = decltype(cv.model)::ExecutionModelFragment; break;
	case ShaderStage::TessControl: cv.type = EShLangTessControl; cv.model = decltype(cv.model)::ExecutionModelTessellationControl; break;
	case ShaderStage::TessEval: cv.type = EShLangTessEvaluation; cv.model = decltype(cv.model)::ExecutionModelTessellationEvaluation; break;
	case ShaderStage::Geometry: cv.type = EShLangGeometry; cv.model = decltype(cv.model)::ExecutionModelGeometry; break;
	case ShaderStage::Compute: cv.type = EShLangCompute; cv.model = decltype(cv.model)::ExecutionModelGLCompute; break;
	}
	return cv;
}
static CompileResult CompileSpirVTo(const spirvbytes& spirv, TargetAPI api, const Options& opt,  APIConversion types) {
	...
	case TargetAPI::OpenGL_ES: return CompileResult{ SPIRVToOpenGL(spirv,opt,types.model) }; break;
	case TargetAPI::Vulkan: if (opt.debug) { return SerializeSPIRV(spirv); } else { return SerializeSPIRV(OptimizeSPIRV(spirv, opt)); } break;
	case TargetAPI::HLSL: return CompileResult{ SPIRVToHLSL(spirv,opt,types.model) }; break;
	case TargetAPI::Metal: return CompileResult{ SPIRVtoMSL(spirv,opt,types.model) }; break;
#ifdef ST_DXIL_ENABLED
	case TargetAPI::DXIL: return CompileResult{ SPIRVToDXIL(spirv,opt,types.model) }; break;
#endif
#ifdef __APPLE__
	case TargetAPI::MetalBinary: return CompileResult{SPIRVtoMBL(spirv,opt,types.model)}; break;
#endif
	case TargetAPI::WGSL: return CompileResult{ SPIRVToWGSL(spirv,opt,types.model) }; break;
	default: throw runtime_error("Unsupported API"); break;
	}
}

// --- [PUBLIC API IMPLEMENTATION] ---
// These are the main functions that our game will call. They are now at the
// end of the file, after all the helper functions they depend on have been defined.
